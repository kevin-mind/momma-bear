...
test.describe('Critical E2E Tests', () => {
  ...
  test('homepage renders products', async ({page}) => {
    ...
    await expect(recommendedProductsHeading).toBeVisible();
  ...
  test('product page loads and renders product details', async ({page}) => {
    ...
    await expect(page.locator('.recommended-products-grid')).toBeVisible({timeout: 10000});
    ...
    if (!productUrl) {
      // If no product URL found, skip this test
      return;
    }
  ...
  test('collections page loads and displays products', async ({page}) => {
    ...
    // If collection link exists, navigate to it
